...
# テキストを画像化する関数
def createImage(str):
    base = Image.open('index.png').convert('RGBA')
txt = Image.new('RGBA',base.size,(255,255,255,0))
    fontSize = 24
    fnt = ImageFont.truetype('AppMincho.otf',fontSize)
    d = ImageDraw.Draw(txt)
    w, h = base.size
    maximumCharCount = int((w - (w % fontSize) ) / fontSize)
    _width = maximumCharCount - 2
    print(_width)
    lines = textwrap.wrap(str,width=_ width)
    _y = h/2 - fontSize*(len(lines) - 1)
    dy = 0

    for line in lines:
        unitsize = d.multiline_textsize(line,font = fnt)
        d. text(((w - unitsize[0])/2, _y + dy),line, font=fnt, fill=(255,255,255))
        dy += fontSize
    out = Image.alpha_composite(base,txt)
    filename = md5(str.encode('utf-8')).hexdigest()
    print(filename)
    out.save("static/" + filename + '.png', 'PNG', quality=100, optimize=True)
    return filename
